...
const list = [
    {
      "id": 1,
      "name": "Kitkat",
      "img": "https://www.bigbasket.com/media/uploads/p/m/40122231_14-nestle-kitkat-share-bag-2-fingers-pack.jpg?tr=w-640,q=80 640w, https://www.bigbasket.com/media/uploads/p/m/40122231_14-nestle-kitkat-share-bag-2-fingers-pack.jpg?tr=w-750,q=80 750w, https://www.",
      "p_id": 22,
      "product_id": 45,
      "s_id": 1,
      "details": "Kitkat Coated Wafer Bar, Share Bag"
    },
    {
      "id": 2,
      "name": "duplo",
    ...
    },
    {
      ...
      "product_id": 104,
      "s_id": 1,
      "details": "Cadbury Dairy Milk"
    }
  ]
const list1 = list.filter((each) => 1 === each.s_id)
const list2 = list.filter((each) => 2 === each.s_id)
const carousel = [
    {
        "id": 1,
        "name": "Kitkat Coated Wafer Bar, Share Bag",
    ...
    {
        "id": 3,
        "name": "smarties-Chewy Fruit Candies",
        "img": "https://i.postimg.cc/FsmzcB6R/Adobe-Stock-623473127-Preview-Editorial-Use-Only.jpg",
        "product_id": 89,
        "p_id": 22,
        "s_id": 1
...
const chocolateList = productList.filter((each) => 22 === each.p_id)
//const cocolateList = productList.filter((each) => sinProduct[0].p_id === each.p_id)
...
class HomeSection extends Component {
    renderSection = () => {
        ...
        return (
            <>
                <Banner img='https://i.postimg.cc/LsKyRYJ8/denny-muller-m-GP8gy-Gb8z-Y-unsplash.jpg' />
                <Carousel key={1} carousel={carousel} bagroundColor="#D3D3D3" />
                <HomeProductCard key={1} list={list1} sectionname="Chocolate Section" bagroundColor="#D3D3D3" />
                <Slider {...settings} className="product-carousal" >
                    {chocolateList.map(each => (
                        <div className="home-product">
                            <HomeProduct key={each.id} details={each} />
                        </div>
                    ))}
                </Slider>
                <Banner img='https://i.postimg.cc/L5Jh74Hd/15554.jpg' />
            </>
        )
